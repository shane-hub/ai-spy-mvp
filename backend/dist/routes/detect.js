...
router.post('/detect', auth_1.optionalAuthenticateToken, upload.single('image'), async (req, res) => {
    try {
        if (!req.file) {
        ...
        if (isGuest) {
            ...
            const usages = await redis_1.default.incr(guestLimitKey);
            ...
            await redis_1.default.expire(guestLimitKey, 365 * 24 * 60 * 60);
        }
        else if (userId) {
        ...
        try {
            ...
            if (userId) {
                await prisma_1.default.scanHistory.create({
                    data: {
                        userId,
